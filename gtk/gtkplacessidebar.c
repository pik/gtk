                   ...
                   gpointer    user_data)
{
  if (!gtk_revealer_get_reveal_child (GTK_REVEALER (self)))
    gtk_widget_hide (GTK_WIDGET (SIDEBAR_ROW (user_data)));
}
...
sidebar_row_class_init (SidebarRowClass *klass)
{
  ...
  gtk_widget_class_bind_template_callback (widget_class, on_button_press_event);
  gtk_widget_class_bind_template_callback (widget_class, on_button_release_event);
}
                  ...
                  gpointer       user_data)
{
  ...
  /* Avoid to open the locations if we are dragging, changing the views
   * etc while dragging is confusing */
                       ...
                       SidebarRow     *row)
{
  ...
  g_object_get (SIDEBAR_ROW (row), "sidebar", &sidebar, "section_type", &section_type, NULL);
  ...
  if (event)
    {
      ...
      if (row)
        {
          g_object_get (SIDEBAR_ROW (row), "label", &label, NULL);
          if (event->button == 1)
            return FALSE;
  ...
  if (place_type_1 == PLACES_CONNECT_TO_SERVER)
  ...
  else
    {
      if (section_type_1 == section_type_2)
        ...
        {
          /* order by section. That means the enum of sections types define the
           * order of them */
